...
void DeleteNode(Data*pHead1,int nDelete)
{
    Data* pCurrentNode =pHead1 ;
    Data* pDeletePrevious = NULL;
    Data* pDeleteNext = NULL;
    int i = 1;
    while (pCurrentNode != NULL)
    {
        if (i==nDelete-1)
        {
            pDeletePrevious = pCurrentNode;
        }
        if (i == nDelete+1)
        {
            pDeleteNext = pCurrentNode;
            break;
        }
        i++;
        pCurrentNode = pCurrentNode->pNext;
    }
    pDeletePrevious->pNext = pDeleteNext;
    pDeleteNext->pPrevious = pDeletePrevious;
